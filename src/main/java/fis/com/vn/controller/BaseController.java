...
public class BaseController {
	...
	public boolean isAllowUrl(HttpServletRequest req, String uri) {
        ...
        for (UserModule tbDanhMucQuyen : danhMucQuyens) {
            if (tbDanhMucQuyen.getUrl() != null && uri.startsWith(tbDanhMucQuyen.getUrl()) && tbDanhMucQuyen.getParentId() != 0) {
	...
	public String sendRequest(MultipartFile fileDangKyKinhDoanh, Map<String, String> allParams, String url, String codeTransaction) {
		try {
			...
			for (Entry<String, String> entry : allParams.entrySet()) {
				builder.addTextBody(entry.getKey(), entry.getValue(), ContentType.TEXT_PLAIN.withCharset(Charset.forName("utf-8")));
			}
			
			builder.addBinaryBody("fileDangKyKinhDoanh", fileDangKyKinhDoanh.getBytes(), ContentType.MULTIPART_FORM_DATA, fileDangKyKinhDoanh.getOriginalFilename() );
